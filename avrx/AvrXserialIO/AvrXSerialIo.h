...
int get_c0(FILE *stream);		// Non blocking, returns -1 when empty
int get_char0(FILE *stream);	// Blocking, always returns character
int put_c0(char c, FILE *stream);		// Non blocking, returns -1 when full
int put_char0(char c, FILE *stream);	// Blocking, always returns 0.

int get_c1(FILE *stream);		// Non blocking, returns -1 when empty
int get_char1(FILE *stream);	// Blocking, always returns character
int put_c1(char c, FILE *stream);		// Non blocking, returns -1 when full
int put_char1(char c, FILE *stream);	// Blocking, always returns 0.

void InitSerial0(uint16_t ubrr);
void InitSerial1(uint16_t ubrr);

extern FILE serial1, serial0;	// File descripters for out simple serial.
