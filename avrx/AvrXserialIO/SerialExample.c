...
//	InitSerial1(BAUD(115200));	// Note: this only works with 12mhz or baud rate crystal
	...
	stderr = &serial1;
	fprintf_P(stderr, PSTR("Hello from Task 1\r"));
	while(1)
	{
		int c;
		BOOL bGotACharacter = FALSE;

		fprintf_P(stderr, PSTR("Type something within 5 seconds\r"));
...
//		while ((c = get_c1()) != EOF)
		while ((c = fgetc(stderr)) != EOF)
		{
			if (!bGotACharacter)
			{
				fprintf_P(stderr, PSTR("You typed in \""));
...
//			myputs_P(put_char1, PSTR("Try again... "));
			fprintf_P(stderr, PSTR("Try again... "));
		else
//			myputs(put_char1, "\"\r");		// Example of non-FLASH string..
			fprintf(stderr, "\"\r");		// Example of non-FLASH string..
	}
}
